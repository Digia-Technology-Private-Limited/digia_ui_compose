 ...
 *
 * [BaseResponse] provides a standardized way to handle API responses, encapsulating success status,
 * data payload, and error information. This allows for consistent error handling and response
 * parsing across different API endpoints.
 *
...
data class BaseResponse<T>(
    ...
    companion object {
         ...
         *
         * This method is used to parse successful API responses into typed objects. The [fromJsonT]
         * function should handle the conversion from raw JSON to the expected type T.
         *
        ...
        fun <T> fromJson(json: Map<String, Any?>, fromJsonT: (Any?) -> T): BaseResponse<T> {
            ...
            return BaseResponse(isSuccess = isSuccess, data = data, error = error)
        }
    }

    /** Converts this response to a JSON string. Similar to the Dart toString() implementation. */
    override fun toString(): String {
        return Json.encodeToString(
                mapOf("isSuccess" to isSuccess, "data" to data, "error" to error)
        )
    }
