...
class NetworkClient(
    ...
    private val mutableProjectHeaders =
    ...
    private val isDebugMode: Boolean
        get() =
                try {
                    ...
                    // Class.forName("com.digia.digiaui.BuildConfig")
    ...
    private fun createDigiaClient(
        ...
        val builder =
                OkHttpClient.Builder().connectTimeout(1000, TimeUnit.SECONDS).addInterceptor { chain
                    ->
                    val request =
                            chain.request()
                                    .newBuilder()
    ...
    suspend fun requestProject(
        ...
        return try {
            val response =
                    requestProjectRaw(bodyType, url, method, additionalHeaders, data, apiName)
            convertToApiResponse(response)
        } catch (e: Exception) {
            ApiResponse.Error(message = e.message ?: "Unknown error", exception = e)
        }
    }
    ...
    private suspend fun requestProjectRaw(
        ...
        return withContext(Dispatchers.IO) {
                                        ...
                                        when (data) {
                                            ...
                                            is Map<*, *> -> {
                                                val jsonObject = buildJsonObject {
                                                    ...
                                                    (data as Map<String, Any?>).entries.forEach {
                                                            (key, value) ->
                                                        when (value) {
                                                            null ->
                                                                    put(
            ...
            val commonKeys =
                    mutableProjectHeaders.keys.intersect(additionalHeaders?.keys ?: emptySet())
            ...
            val requestBuilder =
                    Request.Builder()
                            ...
                            .addHeader(
                                    "Content-Type",
                                    bodyType.contentTypeHeader ?: "application/json"
                            )
    ...
    private fun buildMultipartBody(formData: FormDataBuilder): RequestBody {
        ...
        // Add fields
        formData.fields.forEach { (key, value) -> multipartBuilder.addFormDataPart(key, value) }
        ...
        formData.files.forEach { (key, file) ->
            when (file) {
                is FormFile.LocalFile -> {
                    ...
                    val requestBody = file.file.asRequestBody(mediaType)
    ...
    @Suppress("UNCHECKED_CAST")
    private fun convertToApiResponse(response: OkHttpResponse): ApiResponse<Any> {
        val body = response.body?.string()
        val parsedData =
                if (body != null && body.isNotEmpty()) {
                    try {
                        val jsonElement = jsonParser.parseToJsonElement(body)
                        JsonUtil.jsonElementToAny(jsonElement) as? Map<String, Any>
                    } catch (e: Exception) {
                        mapOf("raw" to body)
                    }
                } else {
            ...
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    BaseResponse(
                            isSuccess = false,
                            data = null,
                            error = mapOf("code" to response.code)
                    )
                }
            } catch (e: IOException) {
    ...
    suspend fun multipartRequestProject(
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                val client =
                        projectClient.newBuilder().connectTimeout(0, TimeUnit.MILLISECONDS).build()
