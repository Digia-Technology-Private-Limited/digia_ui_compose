...
//                        )
        ...
        val trailingIconColor = colors?.trailingIconColor(enabled, isError, isFocused)?:Color.Black
        val decoratedTrailing: @Composable (() -> Unit)? =
            trailingIcon?.let {
                @Composable { Decoration(contentColor = trailingIconColor, content = it) }
            }

        val supportingTextColor = colors?.supportingTextColor(enabled, isError, isFocused)?:Color.Black
