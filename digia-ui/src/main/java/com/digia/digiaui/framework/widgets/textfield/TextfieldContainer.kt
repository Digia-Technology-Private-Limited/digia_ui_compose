...
fun TextFieldContainer(
    ...
    val containerColor = animateColorAsState(
        targetValue = colors?.containerColor(enabled, isError, focused) ?: Color.Transparent,
        animationSpec = tween(durationMillis = 150),
    )

    // Get border color for animation
    val borderColor = when (currentBorder) {
    ...
    Box(
        modifier = modifier
            ...
            .then(
                when (currentBorder) {
                    is VWInputBorder.Outline -> Modifier.outlineBorder(
                        border = currentBorder,
                        color = animatedBorderColor.value,
                        shape = shape
                    )
                    is VWInputBorder.Underline -> Modifier.underlineBorder(
                        border = currentBorder,
                        color = animatedBorderColor.value,
                        focused = focused,
            ...
            onDrawWithContent {
                ...
                if (border.dashed && border.dashPattern != null) {
                    ...
                    drawIntoCanvas { canvas ->
                        ...
                        val scaledPath = if (pathBounds.width > 0 && pathBounds.height > 0) {
                            Path().apply {
                                op(
                                    path,
                                    Path(),
                        ...
                        val paint = android.graphics.Paint().apply {
                            ...
                            strokeCap = when (border.strokeCap) {
                                androidx.compose.ui.graphics.StrokeCap.Butt -> android.graphics.Paint.Cap.BUTT
                                androidx.compose.ui.graphics.StrokeCap.Round -> android.graphics.Paint.Cap.ROUND
                                androidx.compose.ui.graphics.StrokeCap.Square -> android.graphics.Paint.Cap.SQUARE
                                else -> android.graphics.Paint.Cap.BUTT
                            }
                            if (border.dashed && border.dashPattern != null) {
                                pathEffect = android.graphics.DashPathEffect(
                                    border.dashPattern!!.toFloatArray(),
                                    0f
                                )
                            }
                        }
                    ...
                    drawOutline(
                        ...
                        style = Stroke(
                            ...
                            cap = when (border.strokeCap) {
                                androidx.compose.ui.graphics.StrokeCap.Butt -> androidx.compose.ui.graphics.StrokeCap.Butt
                                androidx.compose.ui.graphics.StrokeCap.Round -> androidx.compose.ui.graphics.StrokeCap.Round
                                androidx.compose.ui.graphics.StrokeCap.Square -> androidx.compose.ui.graphics.StrokeCap.Square
...
private fun Modifier.underlineBorder(
    ...
    Modifier.drawWithCache {
        onDrawWithContent {
            ...
            val lineThickness = if (focused) {
                focusedIndicatorLineThickness.toPx()
            } else {
                unfocusedIndicatorLineThickness.toPx()
            }

            var strokeWidth = if (border.strokeWidth.value > 0) {
...
private fun Modifier.customOutlineBorder(
    border: VWInputBorder.Outline,
    color: Color
): Modifier = this.then(
    if (border.strokeWidth.value > 0) {
        Modifier.border(
            width = border.strokeWidth,
            color = color,
            shape = RoundedCornerShape(border.radius)
        )
    } else {
        Modifier
    }
